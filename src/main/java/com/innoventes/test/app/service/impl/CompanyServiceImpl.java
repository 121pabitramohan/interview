...
public class CompanyServiceImpl implements CompanyService {
	...
	public CompanyDTO getCompanyById(long id) {
		Company company = companyRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException(
				String.format(serviceHelper.getLocalizedMessage(ApplicationErrorCodes.COMPANY_NOT_FOUND), id),
				ApplicationErrorCodes.COMPANY_NOT_FOUND));
	...
	@Override
	public Company getCompanyByCompanyId(String companyCode) {
		final Optional<Company> byCompanyCode = companyRepository.getByCompanyCode(companyCode);
		Company company = byCompanyCode.get();
		return company;
	}

	@Override
	public void deleteACompny(Long id) {
